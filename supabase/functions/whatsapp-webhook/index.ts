...
//   listings           (id, title, description, price_chf, area_sqm, rooms, city, canton, postal_code, address, status, primary_url, ...)
...
{"typ":"miete"|"kauf"|null,"city":string|null,"canton":string|null,"min_rooms":number|null,"max_price":number|null,"hat_garten":true|false|null,"seesicht":true|false|null,"zusammenfassung":"kurzer Satz auf Deutsch"}
...
{"aktion":"verfeinern"|"interesse"|"stopp"|"unklar","aktualisierte_filter":{"typ":...,"city":...,"canton":...,"min_rooms":...,"max_price":...,"hat_garten":...,"seesicht":...},"antwort":"kurze deutsche Antwort"}
...
function fallbackParse(text: string): Filter {
  ...
  const min_rooms = zi ? parseFloat(zi[1].replace(",", ".")) : null;
  const preis = t.match(/(\d[\d'']?\d{2,})/);
  const max_price = preis ? parseInt(preis[1].replace(/['']/g, ""), 10) : null;
  return {
    typ,
    city: stadtRaw ? stadtRaw.charAt(0).toUpperCase() + stadtRaw.slice(1) : null,
    canton: null,
    min_rooms,
    max_price,
...
// -------------------------------------------------------------- Listings
async function findMatching(f: Filter, limit = 3): Promise<any[]> {
  let q = supabase
    .from("listings")
    .select("id, title, address, city, canton, postal_code, rooms, area_sqm, price_chf, description, primary_url, image_url, status")
    .neq("status", "rejected")
    .is("archived_at", null)
  ...
  if (f.canton) q = q.ilike("canton", `%${f.canton}%`);
  if (f.min_rooms) q = q.gte("rooms", f.min_rooms);
  if (f.max_price) q = q.lte("price_chf", f.max_price);
  const { data, error } = await q;
  if (error) {
    console.error("findMatching error", error);
    return [];
  }
  return data ?? [];
}

function formatInserat(l: any, typ: "miete" | "kauf"): string {
...
async function handleTextMessage(from: string, text: string): Promise<void> {
  ...
  if (!abo) {
    ...
    const treffer = await findMatching(parsed, 3);
    if (treffer.length === 0) {
    ...
    for (const l of treffer) {
      await sendWhatsApp(from, formatInserat(l, parsed.typ ?? "miete"));
    }
  ...
  if (aktion === "interesse") {
    const treffer = await findMatching(abo.filter_json as Filter, 1);
  ...
  if (aktion === "verfeinern") {
    ...
    await supabase
      ...
      .eq("id", abo.id);
    const treffer = await findMatching(merged, 3);
    if (treffer.length === 0) {
    ...
    for (const l of treffer) {
      await sendWhatsApp(from, formatInserat(l, merged.typ ?? "miete"));
    }
    return;
  }
